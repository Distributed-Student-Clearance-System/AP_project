...
public class RequestDAO {
    ...
    public List<String> getAllRequestsSummary() {
        ...
        String sql =
                "SELECT r.request_id, u.full_name, r.submitted_at, r.status " +
    ...
    private int getRequestCountByStatuses(String... statuses) {
        StringBuilder sql = new StringBuilder(
                "SELECT COUNT(*) AS total_requests FROM clearance_requests WHERE status IN ("
        );
    ...
    public List<ClearanceRequest> getRequestsByStudentId(int studentId) {
        ...
        String sql =
                "SELECT request_id, student_id, submitted_at, status " +
                "FROM clearance_requests " +
                "WHERE student_id = ? " +
                "ORDER BY request_id DESC";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                LocalDateTime submittedAt =
                        ...
                        ? submittedAtTimestamp.toLocalDateTime()
                        : null;
    ...
    private RequestStatus mapRequestStatus(String statusText) {
        ...
        if ("APPROVED".equalsIgnoreCase(statusText)
                || "FULLY_APPROVED".equalsIgnoreCase(statusText)) {
            return RequestStatus.CLEARED;
        }

        return RequestStatus.valueOf(statusText);
    }
